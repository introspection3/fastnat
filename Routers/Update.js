...
router.get('/api/uicheck', async function(req, res, next) {

    let content = await readFile(serverConfigPath);
    let updateConfig = JSON.parse(content).update;

    //ui_linux_arm64_2.2.2.zip
    let version = req.query.version;
    let platform = req.query.platform;
    let arch = req.query.arch;
    let newVersion = updateConfig.uiVersion;
    let baseUrl = updateConfig.baseUrl + '';

    let pkgFileName = `ui_${platform}_${arch}_${newVersion}.zip`;
